...
class TestVirtualFSGet(object):
	...
	class TestGet(object):
		...
		def test_parent_file(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			...
			with pytest.raises(NotADirectoryError):
				vfs.get(p(mock_structure.path / 'fileA' / 'fileAA'), parent_none_is_none = True)
	...
	class TestLink(object):
		...
		def test_parent_file(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.link(p(mock_structure.path / 'fileA'), p(mock_structure.path / 'fileA' / 'fileAA'))

		def test_parent_missing(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.link(p(mock_structure.path / 'fileA'), p(mock_structure.path / 'dir3' / 'file3A'))
	...
	class TestSymlink(object):
		...
		def test_parent_file(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.symlink(p(mock_structure.path / 'dir1'), p(mock_structure.path / 'fileA' / 'dirA1'))

		def test_parent_missing(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.symlink(p(mock_structure.path / 'dir1'), p(mock_structure.path / 'dir3' / 'dir31'))
	...
	class TestHardlink(object):
		...
		def test_parent_file(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.hardlink(p(mock_structure.path / 'fileA'), p(mock_structure.path / 'fileA' / 'fileAA'))

		def test_parent_missing(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			with pytest.raises(NotADirectoryError):
				vfs.hardlink(p(mock_structure.path / 'fileA'), p(mock_structure.path / 'dir3' / 'file3A'))
	...
	class TestScandir(object):
		...
		def test_fake_symlink(self, vfs: VirtualFS, mock_structure: MockStructure) -> None:
			vfs.mkdir(p(mock_structure.path / 'dir3'))
			vfs.symlink(p(mock_structure.path / 'fileA'), p(mock_structure.path / 'dir3' / 'symA'))
			paths = {entry.path for entry in vfs.scandir(p(mock_structure.path / 'dir3'))}
